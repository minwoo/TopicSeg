...
public class LBFGS
{
	...
	public static void lbfgs ( int n , int m , double[] x , double f , double[] g , boolean diagco , double[] diag , int[] iprint , double eps , double xtol , int[] iflag ) throws ExceptionWithIflag
	{
		...
		while ( true )
		{
			...
			{
				iflag[0]=-1;
				throw new ExceptionWithIflag( iflag[0], "Line search failed. See documentation of routine mcsrch. Error return of line search: info = "+info[0]+" Possible causes: function or gradient are incorrect, or incorrect tolerances." + info[0] );
			}
	...
	public static void lb1 ( int[] iprint , int iter , int nfun , double gnorm , int n , int m , double[] x , double f , double[] g , double[] stp , boolean finish )
	{
		...
		if ( iter == 0 )
		{
			...
			if ( iprint [ 2 -1] >= 1 )
			{
				...
				for ( i = 1; i <= n; i++ )
					System.out.print(String.format("  %.2f", Math.exp(x[i-1])) );
				System.out.println( "" );
		...
		else
		{
			...
			if ( iprint [ 2 -1] == 2 || iprint [ 2 -1] == 3 )
			{
				...
				for ( i = 1; i <= n; i++ )
					System.out.print(String.format("  %.2f", Math.exp(x[i-1])) );
					//System.out.print( "  "+Math.exp(x[i-1]) );
				System.out.println( "" );
				if ( iprint [ 2 -1] == 3 )
				{
					System.out.print( " gradient vector g =" );
